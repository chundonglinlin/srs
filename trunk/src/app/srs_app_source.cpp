...
srs_error_t SrsOriginHub::on_video(SrsSharedPtrMessage* shared_video, bool is_sequence_header)
{
    ...
    if (format->is_avc_sequence_header()) {
        ...
        if (c->id == SrsVideoCodecIdAVC) {
            ...
            err = stat->on_video_info(req_, c->id, c->hevc_dec_conf_record_.general_profile_idc, c->hevc_dec_conf_record_.general_level_idc, c->width, c->height);
            srs_trace("%dB video sh, codec(%d, profile=%s, level=%s, %dx%d, %dkbps, %.1ffps, %.1fs)",
                msg->size, c->id, srs_hevc_profile2str((SrsHevcProfile)c->hevc_dec_conf_record_.general_profile_idc).c_str(),
                srs_hevc_level2str((SrsHevcLevel)c->hevc_dec_conf_record_.general_level_idc).c_str(),
                c->width, c->height, c->video_data_rate / 1000, c->frame_rate, c->duration);
#endif
        }
        if (err != srs_success) {
