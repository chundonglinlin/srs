...
string SrsRtspSdp::to_strings()
{
    ...
    m=video 0 RTP/AVP 96
    a=rtpmap:96 H264/90000
    a=fmtp:96 packetization-mode=1; sprop-parameter-sets=Z2QAKKzZQHgCJ+XARAAAAwH0AABdqDxgxlg=,aOvjyyLA; profile-level-id=640028
    a=control:streamid=0
...
srs_error_t SrsRtspResponse::encode(stringstream& ss)
{
    ...
    // content.
    if ((err = encode_content(ss)) != srs_success) {
...
srs_error_t SrsRtspResponse::encode_content(std::stringstream& ss)
{
    return srs_success;
}
...
srs_error_t SrsRtspDescribeResponse::encode_header(stringstream& ss)
{
    ...
    if(!str.empty()){
        ...
        ss << "Content-Length: " << str.size() << SRS_RTSP_CRLF;
    }

    return srs_success;
}

srs_error_t SrsRtspDescribeResponse::encode_content(stringstream& ss)
{
    string str = sdp->to_strings();
    if(!str.empty()){
        // sdp
...
SrsRtspPlayResponse::SrsRtspPlayResponse(int seq) : SrsRtspResponse(seq)
{
}
